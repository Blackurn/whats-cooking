...
import { loadWorkflow, WorkflowWatcher, resolveWorkflowPath } from './workflow';
...
export class Orchestrator {
  ...
  private watcher: WorkflowWatcher | null = null;
  ...
  private notify(type: OrchestratorEventType, data?: unknown): void {
  ...
  private async startupCleanup(): Promise<void> {
    if (!this.tracker || !this.config) return;
    try {
      ...
      for (const issue of terminalIssues) {
        if (this.workspaceManager?.workspaceExists(issue.identifier)) {
          logger.info(`startup_cleanup_removing_workspace`, { issue_identifier: issue.identifier });
          await this.workspaceManager.removeWorkspace(issue.identifier);
        }
      }
    } catch (e) {
      logger.warn(`startup_cleanup_failed err=${String(e)}`);
    }
  }

  private applyWorkflow(def: WorkflowDefinition): void {
    const workflowDir = path.dirname(this.workflowPath);
    const newConfig = parseConfig(def.config, workflowDir);
    const validation = validateConfig(newConfig);
    if (!validation.valid) {
      throw new SymphonyError('workflow_parse_error', `Workflow validation failed: ${validation.errors.join('; ')}`);
    }

    const newTracker = createTracker(newConfig.tracker);
    const newWorkspaceManager = new WorkspaceManager(newConfig);
    ensureWorkspaceRoot(newConfig);
  ...
  private reloadWorkflowKeepingLastGood(): boolean {
    try {
      const freshDef = loadWorkflow(this.workflowPath);
      this.applyWorkflow(freshDef);
      return true;
    } catch (e) {
      logger.error(`workflow_reload_error msg=${String(e)}`);
      this.notify('error', e);
      return false;
    }
  }
  ...
  async start(): Promise<void> {
    ...
    try {
      def = loadWorkflow(this.workflowPath);
    } catch (e) {
      logger.error(`startup_failed err=${String(e)}`);
      throw e;
    }
    ...
    if (!v.valid) {
      ...
      logger.error(msg);
      throw new SymphonyError('missing_tracker_api_key', msg);
    }
    ...
    this.watcher = new WorkflowWatcher(
      ...
      (err) => {
        logger.error(`workflow_reload_error code=${err.code} msg=${err.message}`);
    ...
    this.scheduleTick(0);
    logger.info(`orchestrator_started workflow=${this.workflowPath}`);
  }
  ...
  stop(): void {
    ...
    for (const [issueId, entry] of this.state.running) {
      logger.info(`orchestrator_stopping_worker`, { issue_id: issueId, issue_identifier: entry.issue.identifier });
      entry.abort?.();
    }
  ...
  private async tick(): Promise<void> {
    ...
    if (!config) {
      logger.warn('tick_skipped reason=no_config');
    ...
    try {
      ...
      const v = validateConfig(effectiveConfig);
      if (!v.valid) {
        logger.error(`dispatch_skipped reason=validation_failed errors=${v.errors.join('; ')}`);
        this.notify('error', v);
      ...
      try {
        ...
        logger.error(`candidate_fetch_failed err=${String(e)}`);
        this.scheduleTick(effectiveConfig.polling.intervalMs);
        return;
      }
      ...
      for (const issue of sorted) {
        if (this.availableSlots(effectiveConfig) <= 0) break;
        if (this.isEligible(issue, effectiveConfig)) {
          this.dispatch(issue, null, effectiveConfig);
        }
      }
    } catch (e) {
      logger.error(`tick_error err=${String(e)}`);
    }
  ...
  private async reconcile(config: ServiceConfig): Promise<void> {
    ...
    if (config.codex.stallTimeoutMs > 0) {
      ...
      for (const [issueId, entry] of this.state.running) {
        ...
        if (elapsed > config.codex.stallTimeoutMs) {
          logger.warn(`stall_detected elapsed=${elapsed}ms`, {
            issue_id: issueId,
    ...
    try {
      ...
      logger.warn(`reconcile_state_refresh_failed err=${String(e)}`);
    ...
    for (const [issueId, entry] of this.state.running) {
      ...
      if (isTerminal) {
        logger.info(`reconcile_terminating issue_state=${currentState}`, {
          issue_id: issueId,
          issue_identifier: entry.issue.identifier,
        });
        ...
        logger.info(`reconcile_stopping_non_active issue_state=${currentState}`, {
          issue_id: issueId,
          issue_identifier: entry.issue.identifier,
        });
  ...
  private terminateWorker(issueId: string, reason: string): void {
    ...
    if (!entry) return;
    logger.info(`worker_terminating reason=${reason}`, {
      issue_id: issueId,
      issue_identifier: entry.issue.identifier,
    });
    entry.abort?.();
  ...
  private perStateSlots(state: string, config: ServiceConfig): number {
    const key = state.toLowerCase();
    const cap = config.agent.maxConcurrentAgentsByState[key];
    if (cap === undefined) return this.availableSlots(config);
  ...
  private isEligible(issue: Issue, config: ServiceConfig): boolean {
    ...
    if (!activeNorm.includes(stateNorm)) return false;
    if (terminalNorm.includes(stateNorm)) return false;
    if (this.state.running.has(issue.id)) return false;
    if (this.state.claimed.has(issue.id)) return false;
    if (this.availableSlots(config) <= 0) return false;
    if (this.perStateSlots(issue.state, config) <= 0) return false;

    // Blocker rule: Todo issues blocked by non-terminal issues don't dispatch
    if (stateNorm === 'todo') {
      for (const blocker of issue.blockedBy) {
        const bState = blocker.state?.toLowerCase() ?? '';
        if (!terminalNorm.includes(bState)) return false;
      }
    }
  ...
  private dispatch(issue: Issue, attempt: number | null, config: ServiceConfig): void {
    ...
    logger.info(`dispatching attempt=${attempt ?? 0}`, {
      issue_id: issue.id,
  ...
  private async runWorker(
      ...
      onEvent: (event: AgentEvent) => {
        ...
        const rateLimits = event.payload?.['rateLimits'] ??
          (typeof params === 'object' && params !== null
            ...
            : undefined);
        if (rateLimits) {
          this.state.codexRateLimits = rateLimits as Record<string, unknown>;
        }
    ...
    if (result.success) {
      logger.info(`worker_succeeded turns=${result.finalTurnCount}`, ctx);
      this.state.completed.add(issue.id);
      // Schedule continuation retry (Section 7.1)
      if (this.running) this.scheduleRetry(issue.id, issue.identifier, 1, false, null, config);
    } else {
      logger.warn(`worker_failed err=${result.error ?? 'unknown'}`, ctx);
      const currentAttempt = (attempt ?? 0) + 1;
      if (this.running) this.scheduleRetry(issue.id, issue.identifier, currentAttempt, true, result.error ?? 'worker_failed', config);
    }
  ...
  private scheduleRetry(
    ...
    logger.info(`retry_scheduled attempt=${attempt} delay=${delay}ms`, {
      issue_identifier: identifier,
    });

    const timerHandle = setTimeout(
      () => void this.handleRetryFired(issueId, identifier, attempt, config),
      delay,
    );
  ...
  private async handleRetryFired(
    ...
    try {
      ...
      logger.error(`retry_fetch_failed err=${String(e)}`, ctx);
    ...
    if (!issue) {
      logger.info(`retry_release reason=not_found_in_active_candidates`, ctx);
      this.state.claimed.delete(issueId);
      return;
    }
    ...
    if (!activeNorm.includes(stateNorm)) {
      logger.info(`retry_release reason=no_longer_active issue_state=${issue.state}`, ctx);
      this.state.claimed.delete(issueId);
      return;
    }
    ...
    if (this.availableSlots(config) <= 0 || this.perStateSlots(issue.state, config) <= 0) {
      ...
      logger.warn(`retry_requeued reason=${errMsg}`, ctx);
