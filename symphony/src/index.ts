...
async function main(): Promise<void> {
  ...
  let httpPort: number | undefined = cliPort;
  ...
  if (httpPort !== undefined && !isNaN(httpPort)) {
    try {
      const srv = await startHttpServer(orchestrator, httpPort);
      logger.info(`http_server_ready address=${srv.address()}`);
    } catch (e) {
      logger.error(`http_server_failed err=${String(e)}`);
    }
  }
  ...
  const shutdown = (signal: string) => {
    ...
    shuttingDown = true;
    logger.info(`shutdown_signal signal=${signal}`);
    orchestrator.stop();
    process.exit(0);
  };
